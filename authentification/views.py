...
from .Projet_spam.spamPredHeatmap import df_tot_heatmap, df_tot_time, plot_heatmap_spam, plot_time_heatmap
from .Projet_spam.predNouvSms import Pred_news_spams, loaded_model_MNB
# import io
...
def classification(request):
    ...
    if request.method == 'POST' and 'colonne' in request.POST:
        ...
        df_dict = request.session.get('df') # récupération le df sous la forme d'un dictionnaire à partir de la session de l'utilisateur
...
def projetspam(request):
    ...
    # plot_heatmap_spam
    fig_heatM = plot_heatmap_spam(df = df_tot_heatmap)
    graph_heatM = plot(fig_heatM, output_type='div')
    
    fig_time_heat = plot_time_heatmap(df = df_tot_time)
    graph_time_heat = plot(fig_time_heat, output_type='div')
    ...
    if request.method == 'POST' and 'file' in request.POST:
        ...
        # Lire les données du fichier CSV et les convertir en un dataframe
        try:
            df = pd.read_csv(fichier, sep='\t',header=None, names=['type', 'text'])
            # df = pd.read_csv('authentification/Projet_spam/spam.csv',encoding = "latin-1")
            # df = pd.DataFrame(data)
            #print(df)
            if any(df['type'].str.len() > 5):
                df = df.rename(columns={'type': 'text', 'text' : 'type'})
                df['type'] = '?'
                print(df.head())
            request.session['df'] = df.to_dict()  # le dataframe est enregistré dans la session de l'utilisateur
            ...
            'df': df,
            "files": files,
            "graph_heatM" : graph_heatM,
            "graph_time_heat" : graph_time_heat}
    ...
    if request.method == 'POST' and 'MNB' in request.POST:
        fichier = request.session.get('fichier')
        selected_column = request.session.get('selected_column')
        df_dict = request.session.get('df') # récupération du df sous la forme d'un dictionnaire à partir de la session de l'utilisateur
        if df_dict is not None:
            df = pd.DataFrame.from_dict(df_dict)
        selected_column = request.session.get('selected_column') # récupération de la colonne sélectionnée à partir de la session de l'utilisateur
        ...
        context = {'files': files,
                ...
                'df': df,
                'selected_column': selected_column,
                "data_news_sms_pred" : data_news_sms_pred,
                'data_json': data_json,
                "graph_heatM" : graph_heatM,
                "graph_time_heat" : graph_time_heat
                }
    
    # Si on entre dans cette page pour la première fois
    elif request.method == 'GET':
        context = {"files": files,
                   'df': df, 
                   "graph_heatM" : graph_heatM,
